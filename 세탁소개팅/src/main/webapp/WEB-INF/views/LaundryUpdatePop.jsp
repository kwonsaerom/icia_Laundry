...
<!-- JAVASCRIPTS --> 
<script type="text/javascript" src="resources/bootstrap/layout/scripts/jquery.min.js"></script> 
<script>
   var arrdelete = [];
   var arr = [];
   var arrinsert = [];
   ...
   function addHandle() {
      $('#itemadd input').each(function(i) {//추가된 아이템
         arrinsert.push($(this).val());
      });
      arrStrinsert = arrinsert.toString();
      $('#arrStrinsert').val(arrStrinsert);
      
      $('#orgitem input').each(function(i) {//원래있던 아이템
         arr.push($(this).val());
      });
      arrStr = arr.toString();
      $('#arr').val(arrStr);
      
      arrStrdelete = arrdelete.toString();//DB에서 뽑아온 리스트 중 삭제된 아이템
      $('#arrStrdelete').val(arrStrdelete);
      
         
      var nl_code = $("#nl_code").val();
      var nl_name = $("#nl_name").val();
      var nl_address = $("#nl_address").val();
      var post = $("#post").val();
      var addr1 = $("#addr1").val();
      var addr2 = $("#addr2").val();
      var first = $("#first").val();
      ...
      var kind = $("#kind").val();
      //location.href="./possibleItemUpdate?nl_address="+nl_address+"&nl_name="+nl_name+"&post="+post+"&addr1="+addr1+"&addr2="+addr2+"&first="+first+"&second="+second+"&third="+third+"&nl_open="+nl_open+"&nl_close="+nl_close+"&nl_content="+nl_content+"&nl_timelap="+nl_timelap+"&nl_qty="+nl_qty;
      document.HandleForm.action="./possibleItemUpdate?nl_address="+nl_address+"&nl_name="+nl_name+"&post="+post+"&addr1="+addr1+"&addr2="+addr2+"&first="+first+"&second="+second+"&third="+third+"&nl_open="+nl_open+"&nl_close="+nl_close+"&nl_content="+nl_content+"&nl_timelap="+nl_timelap+"&nl_qty="+nl_qty;
...
<div class="wrapper" style="background-color:white; height: 90%;">
   ...
   <form name="HandleForm" target="LaundryUpdate.jsp" style="height:100%;">
      ${lhListInput}
      ${orghandleupdate }
      <table id="itemadd">
         <tr id="item${num}"></tr>
      ...
      <input type="hidden" name="arrStrdelete" id="arrStrdelete"/>
      <input type="hidden" name="arrStrinsert" id="arrStrinsert"/>
      <div style="width: 100px;margin-left: 608px;margin-top: 12px;">
         <a href="#" onclick="addHandle()" class="button" >등록 </a>
      </div>
      <input type="hidden" id="nl_code" name="nl_code" value="${nl_code}"/>
      <input type="hidden" id="nl_name" name="nl_name" value="${nlMap.nl_name}"/>
      <input type="hidden" id="nl_address" name="nl_address" value="${nlMap.nl_address}"/>
...
<script>
   ...
   function removerealitem(elem){
      $('#item'+elem+' input').each(function(i) {//추가된 아이템
         arrdelete.push($(this).val());
      });
      console.log(arrdelete);
      console.log(arrdelete[0]);
